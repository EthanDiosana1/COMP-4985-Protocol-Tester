...
// Define test cases
START_TEST(test_addition_positive_integers)    // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
{
    // Test add function with two positive integers
    ck_assert_int_eq(add(3, 4), 7);
}

START_TEST(test_addition_positive_and_negative_integer)    // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
{
    // Test add function with one positive and one negative integer
    ck_assert_int_eq(add(3, -4), -1);
}

START_TEST(test_subtraction_positive_integers)    // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
{
    // Test subtract function with two positive integers
    ck_assert_int_eq(subtract(10, 4), 6);
}

START_TEST(test_subtraction_positive_and_negative_integer)    // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
{
    // Test subtract function with one positive and one negative integer
    ck_assert_int_eq(subtract(10, -4), 14);
}

START_TEST(test_subtraction_negative_result)    // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
{
